...
module.exports.login = async (req, res) => {
     ...
     if(errors.isEmpty()) {
             ...
             if(user) {
                  if(await comparePassword(password, user.password)) {
                     ...
                     if(user.admin) {
                      ...
                      return res.status(400).json({errors: [{msg: 'Email hoặc mật khẩu không chính xác!', param: 'password'}]})
                  }
             } else {
                 return res.status(400).json({errors: [{msg: 'Email hoặc mật khẩu không chính xác!', param: 'email'}]});
             }
         } catch (error) {
             console.log(error.message)
             return res.status(500).json('Server internal error!');
        ...
        //  validations failed
        return res.status(400).json({errors: [{msg: 'Bạn chưa nhập email hoặc mật khẩu!', param: 'email'}]})
     }
}