  ...
  async paymentProcess(req, res, next) {
    ...
    const session = await stripe.checkout.sessions.create({
      shipping_address_collection: {
        allowed_countries: ["VN", "US", "CA", "AU", "SG", "CN"],
      },
      shipping_options: [
        {
          shipping_rate_data: {
            ...
            // Chọn sự kiện giao hàng từ 5-7 ngày làm việc
            delivery_estimate: {
              minimum: {
                unit: "business_day",
                value: 5,
  ...
  async checkOutSession(request, response) {
    ...
    switch (event.type) {
      case "payment_intent.succeeded":
        ...
        // Sau đó, xác định và gọi hàm để xử lý sự kiện Payment_intent.succeeded
        break;
    ...
    // Trả lại 200 phản hồi để xác nhận đã nhận sự kiện
    response.send();
  }
  async paymentVerify(req, res) {
    ...
    try {
      ...
      return res.status(200).json({
        msg: "Bạn đã thanh toán thành công!",
