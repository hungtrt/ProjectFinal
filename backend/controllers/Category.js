...
class Category {
  async create(req, res) {
    ...
    if (errors.isEmpty()) {
      ...
      if (!exist) {
        ...
        return res.status(201).json({ message: "Danh mục được tạo thành công!" });
      } else {
        return res.status(400).json({ errors: [{ msg: `Lỗi! Danh mục '${name}' đã tồn tại.` }] });
      }
    } else {
  ...
  async deleteCategory(req, res) {
    ...
    try {
      ...
      return res.status(200).json({ message: "Đã xóa thành công danh mục!" });
    } catch (error) {
      console.log(error.message);
