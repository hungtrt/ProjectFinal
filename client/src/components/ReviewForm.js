          ...
          <div className="bg-white p-5">
            ...
            {response.isError &&
              response?.error?.data?.errors.map((err) => (
                <p key={err} className="bg-rose-50 px-4 py-2.5 text-rose-900 rounded mb-2 font-medium">
                  {err.msg}
                </p>
              ))}
            <form onSubmit={addReview}>
              <div className="mb-3">
                <label htmlFor="rating" className="mb-2 capitalize font-medium text-sm block">
                  số sao
                </label>
              ...
              <div className="mt-3 flex justify-between">
                <input type="submit" value="add reivew" className="btn-indigo rounded" />
                <button
